...
import { Search, Folder, Pin, MessageSquare, ChevronDown, ChevronRight } from "lucide-react";
        ...
        <div className="flex flex-col">
          {existingFolders.length === 0 ? (
            ...
            existingFolders.map((folder, idx) => (
              <div key={folder} className="flex flex-col border-b border-slate-100 dark:border-slate-800/50 last:border-0">
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-8 h-8 rounded-md bg-slate-200 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400">
                      {expandedFolder === folder ? <ChevronDown size={16} /> : <Folder size={16} />}
                    </div>
                  ...
                  <div className="flex flex-col gap-1 pl-14 pr-4 py-2 mb-2">
                    {chats.filter(c => c.folder === folder).map(chat => (
                      <div 
                        ...
                        className="flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md cursor-pointer transition-colors"
                      >
